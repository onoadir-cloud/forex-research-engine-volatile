#!/usr/bin/env python3
"""Research-only Daily Box Session-Open Quality-Filter Sweep/Reclaim study.
...
MIN_SWEEP_DEPTH_PIPS = {"JPY": 5.0, "NON_JPY": 3.0}
MIN_BOX_RANGE_PIPS = {"JPY": 40.0, "NON_JPY": 30.0}
MAX_BOX_RANGE_PIPS = {"JPY": 250.0, "NON_JPY": 180.0}
MIN_RECLAIM_FRACTION = 0.20
EVENT_COLUMNS = [
    ...
    "box_range_pips", "setup_direction", "sweep_datetime", "sweep_depth_pips", "reclaim_strength_fraction",
    "rejection_wick_pips", "sweep_body_pips", "confirmation_datetime", "entry_price",
...
def symbol_family(symbol: str) -> str:
    return "JPY" if "JPY" in symbol.upper() else "NON_JPY"
...
def candle_body_pips(row: pd.Series, pip_size: float) -> float:
    return float(abs(row["close"] - row["open"]) / pip_size)


def lower_wick_pips(row: pd.Series, pip_size: float) -> float:
    return float((min(row["open"], row["close"]) - row["low"]) / pip_size)


def upper_wick_pips(row: pd.Series, pip_size: float) -> float:
    return float((row["high"] - max(row["open"], row["close"])) / pip_size)


def detect_setup(scan_df: pd.DataFrame, direction: str, box: dict, symbol: str, pip_size: float) -> dict | None:
    box_bottom = float(box["box_bottom"])
    box_top = float(box["box_top"])
    box_range = box_top - box_bottom
    if scan_df.empty or box_range <= 0:
        return None

    family = symbol_family(symbol)
    min_depth_pips = MIN_SWEEP_DEPTH_PIPS[family]

    sweep_pos: int | None = None
    sweep_quality: dict[str, float] | None = None
    if direction == "long":
        for pos, (_, row) in enumerate(scan_df.iterrows()):
            if row["low"] > box_bottom:
                continue
            depth_pips = float((box_bottom - row["low"]) / pip_size)
            reclaim_fraction = float((row["close"] - box_bottom) / box_range)
            body = candle_body_pips(row, pip_size)
            wick = lower_wick_pips(row, pip_size)
            close_inside = box_bottom < row["close"] < box_top
            if close_inside and depth_pips >= min_depth_pips and reclaim_fraction >= MIN_RECLAIM_FRACTION and wick >= body:
                sweep_pos = pos
                sweep_quality = {
                    "sweep_depth_pips": depth_pips,
                    "reclaim_strength_fraction": reclaim_fraction,
                    "rejection_wick_pips": wick,
                    "sweep_body_pips": body,
                }
            break
    else:
        for pos, (_, row) in enumerate(scan_df.iterrows()):
            if row["high"] < box_top:
                continue
            depth_pips = float((row["high"] - box_top) / pip_size)
            reclaim_fraction = float((box_top - row["close"]) / box_range)
            body = candle_body_pips(row, pip_size)
            wick = upper_wick_pips(row, pip_size)
            close_inside = box_bottom < row["close"] < box_top
            if close_inside and depth_pips >= min_depth_pips and reclaim_fraction >= MIN_RECLAIM_FRACTION and wick >= body:
                sweep_pos = pos
                sweep_quality = {
            ...
            break
    if sweep_pos is None or sweep_quality is None:
        return None
...
def scan_symbol(symbol: str, path: Path, cfg: Config) -> tuple[pd.DataFrame, list[str]]:
    ...
    for day, day_df in df.groupby("date", sort=True):
        ...
        box = boxes[day]
        family = symbol_family(symbol)
        if box["box_range_pips"] < MIN_BOX_RANGE_PIPS[family] or box["box_range_pips"] > MAX_BOX_RANGE_PIPS[family]:
            continue
        for session_name in SYMBOL_SESSION_MAP[symbol]:
            ...
            candidates = [c for c in (detect_setup(scan_df, "long", box, symbol, pip_size), detect_setup(scan_df, "short", box, symbol, pip_size)) if c is not None]
            if not candidates:
                continue
            for target_mode in TARGET_MODES:
                ...
                events.append({
                    ...
                    "sweep_depth_pips": chosen["sweep_depth_pips"],
                    "reclaim_strength_fraction": chosen["reclaim_strength_fraction"],
                    "rejection_wick_pips": chosen["rejection_wick_pips"],
                    "sweep_body_pips": chosen["sweep_body_pips"],
...
def verdict(wf: pd.DataFrame, by_month: pd.DataFrame, cfg: Config) -> tuple[pd.DataFrame, str, str]:
    ...
    if pass_core:
        label, recommendation, reasons = "PASS", "proceed to locked robustness, not EA", ["OOS passes profitability, edge, observation, ambiguity, concentration, and risk filters."]
    elif fail_reasons:
        label, recommendation, reasons = "FAIL", "reject Daily Box family entirely", fail_reasons
    else:
        label, recommendation, reasons = "WARN", "do not change prior rejection without locked robustness", warn_reasons or ["OOS is mixed."]
...
def write_summary(
    ...
    tail_risk = by_symbol_session_target.sort_values(
        ["p05_net_pnl", "max_drawdown_pips"], ascending=[True, True]
    ) if not by_symbol_session_target.empty else by_symbol_session_target

    with path.open("w", encoding="utf-8") as handle:
        handle.write("# Daily Box Quality-Filter Sweep/Reclaim Research Summary\n\n")
        handle.write("## Research framing\n")
        handle.write("- This is the final pre-defined quality-filter test after prior broad Daily Box Session-Open failures and locked post-hoc candidate robustness failures.\n")
        handle.write("- This is not optimization, not candidate mining, not a Top-20 report, not EA development, and not live-trading logic.\n")
        handle.write("- The previous Daily Box rejection decision is not changed by this script alone.\n")
        handle.write("- If this quality-filter test fails, the Daily Box family should be rejected entirely.\n")
        handle.write("- If this quality-filter test passes, the next step is locked robustness testing, not EA implementation.\n\n")
        ...
        handle.write("- Execution uses M15 candles only for sweep/reclaim detection, confirmation, and trade management.\n")
        handle.write("- Symbols, sessions, target modes, cost assumptions, and walk-forward splits match the broad Daily Box Session-Open family study.\n\n")
        handle.write("## Strict quality filters\n")
        handle.write("- Previous daily box range filter: JPY boxes must be 40 to 250 pips; non-JPY boxes must be 30 to 180 pips.\n")
        handle.write("- Long bottom sweep/reclaim: sweep candle low must breach Box_Bottom, close back inside the box, reach minimum depth, close at least 20% of the box range above Box_Bottom, and have lower wick pips >= body pips.\n")
        handle.write("- Short top sweep/reclaim: sweep candle high must breach Box_Top, close back inside the box, reach minimum depth, close at least 20% of the box range below Box_Top, and have upper wick pips >= body pips.\n")
        handle.write("- Minimum sweep depth: 5 pips for JPY pairs and 3 pips for non-JPY pairs.\n")
        handle.write("- Confirmation remains unchanged: longs require close > prior in-box green candle high; shorts require close < prior in-box red candle low.\n\n")
        ...
        handle.write("## Symbol/session/target summary\n")
        handle.write(markdown_table(by_symbol_session_target, 30) + "\n")
        ...
        handle.write("## Monthly concentration check\n")
        month_cols = ["month", "trades", "expectancy", "profit_factor", "total_net_pnl", "mean_edge_vs_random", "mean_edge_vs_continuation", "max_drawdown_pips"]
        handle.write(markdown_table(by_month[[c for c in month_cols if c in by_month.columns]], 40) + "\n")
        handle.write("## Tail-risk check\n")
        ...
        handle.write(markdown_table(tail_risk[[c for c in tail_cols if c in tail_risk.columns]], 20) + "\n")
        handle.write("## Walk-forward interpretation\n")
        wf_cols = ["segment", "from_year", "to_year", "symbol", "session_name", "target_mode", "trades", "expectancy", "profit_factor", "total_net_pnl", "mean_edge_vs_random", "mean_edge_vs_continuation", "max_drawdown_pips", "longest_loss_streak"]
        handle.write(markdown_table(wf[[c for c in wf_cols if c in wf.columns]], 40) + "\n")
        ...
        handle.write("A PASS only authorizes locked robustness research; it does not authorize EA development. A FAIL rejects the Daily Box family entirely.\n\n")
        if warnings:
...
def parse_args() -> Config:
    parser = argparse.ArgumentParser(description="Research-only Daily Box Session-Open quality-filter sweep/reclaim study.")
    parser.add_argument("--data-dir", default="data")
    parser.add_argument("--output-dir", default="reports/daily_box_quality_filter")
...
def run_research(cfg: Config) -> dict[str, Path]:
    ...
    outputs = {
        "events": cfg.output_dir / "daily_box_quality_events.csv",
        "by_symbol_session_target": cfg.output_dir / "daily_box_quality_by_symbol_session_target.csv",
        "by_symbol": cfg.output_dir / "daily_box_quality_by_symbol.csv",
        "by_session": cfg.output_dir / "daily_box_quality_by_session.csv",
        "by_target": cfg.output_dir / "daily_box_quality_by_target.csv",
        "by_year": cfg.output_dir / "daily_box_quality_by_year.csv",
        "by_month": cfg.output_dir / "daily_box_quality_by_month.csv",
        "walkforward": cfg.output_dir / "daily_box_quality_walkforward.csv",
        "verdict": cfg.output_dir / "daily_box_quality_verdict.csv",
        "summary": cfg.output_dir / "daily_box_quality_summary.md",
    }
...
def main() -> None:
    ...
    print("Research-only Daily Box Quality-Filter study complete.")
